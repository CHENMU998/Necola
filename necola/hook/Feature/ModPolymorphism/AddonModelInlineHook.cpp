...
namespace F {
    ...
    const DWORD AddonModelInlineHook::MELEE_TARGET_OFFSET = 0x1A1;
    const DWORD AddonModelInlineHook::MELEE_RETURN_OFFSET = 0x1AA;

    const DWORD AddonModelInlineHook::WEAPON_TARGET_OFFSET = 0x230;
    const DWORD AddonModelInlineHook::WEAPON_RETURN_OFFSET = 0x242;
            ...
            if(pCharacter) {
                ...
                if(pCombatWeapon) {
                    if(G::WeaponPoly.containsEntity(pCombatWeapon->entindex())) {
                        const model_t* pModel = I::ModelInfo->GetModel(pCombatWeapon->m_iWorldModelIndex());
                        if (pModel) {
    ...
    void AddonModelInlineHook::SetBaseFunctionAddress(DWORD baseAddress) {
        ...
        if (!m_meleeAdoonModelIsHooked) {
            ...
            g_MeleeAddonModelReturnAddress = m_MeleeAddonModelReturnAddress;
        }
